    ...
    if r.legs_out {
        for f in r.feet(cfg) {
            ...
            for (pi, p) in pads.iter().enumerate() {
                // Feet only catch on the TOP face. Solidity from below (and the
                // body slamming the pad) is handled by the static pad-capsule
                // collision, so the feet stay one-sided: a foot resting deep in
                // the slab is pushed up rather than ejected out the bottom (which
                // used to drop the rocket onto its hull and clip the legs).
                let pen = p.top() - f.y; // depth below the top face
